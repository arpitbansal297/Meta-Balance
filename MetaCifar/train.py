...
if __name__ == '__main__':
    ...
    if args.method == 'metabalance':
        ...
        elif args.dataset_type == 'long_tail':
            gamma = 1000
            class_num = 10
            mu = np.power(1 / gamma, 1 / (class_num - 1))
            fractions = np.ones(10) / 1000
            for i in range(class_num):
                fractions[i] = np.power(mu, i)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            model_test = copy.deepcopy(model)
            model_test = model_test.cuda()
            save_file = save_file_folder + "_" + args.method + "_" + str(args.seed) + '.pth'
            torch.save({
                'epoch': epoch,
                'model_state_dict': model_test.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    if args.method == 'metabalance_2':
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            for i, data in enumerate(train_outer_dataloader, 0):
                ...
                # accumulate the loss
                batch_losses.append(loss + 0.005*loss_leak)
...
###################################################
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            model_train_accuracy /= len(trainloader)
            print("Epoch : ", epoch)
            print("Test : ", test_accuracy)
        ...
        trainloader, _, testloader = get_loaders(args)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            for i, data in enumerate(trainloader, 0):
                ...
                inputs, labels = data
                inputs, labels = inputs.cuda(), labels.cuda()
                orig_inputs, orig_labels = inputs, labels

                inputs, targets_a, targets_b, lam = mixup_data(inputs, labels, alpha, True)
                ...
                ##### consistency and hence compute on original batch of images #####
                inputs, labels = orig_inputs, orig_labels
                outputs = model(inputs)
                loss = criterion(outputs, labels)
                loss_element = criterion_element(outputs, labels)
